...
if [ "$platform_os" = "Linux" ]; then
    ...
    platform_dist=`cat /etc/os-release | grep -E "^ID=" | cut -d '"' -f 2`
    platform_vnum=`cat /etc/os-release | grep -E "^VERSION_ID=" | cut -d '"' -f 2`
    ...
    elif [ "$platform_dist" = "Debian" ]; then

        platform_desc='debian'
